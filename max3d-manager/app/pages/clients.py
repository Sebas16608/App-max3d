...
class ClientsPage(QWidget):
    ...
    def refresh(self):
        ...
        for i, c in enumerate(clientes):
            ...
            btn_edit.clicked.connect(lambda ch, cid=c["id"]: self.edit_cliente(cid))
            btn_layout.addWidget(btn_edit)

            btn_history = QPushButton("Historial")
            btn_history.setObjectName("action")
            btn_history.setFixedHeight(32)
            btn_history.clicked.connect(lambda ch, cid=c["id"]: self.show_history(cid))
            btn_layout.addWidget(btn_history)

            btn_del = QPushButton("Borrar")
            btn_del.setObjectName("action-del")
            btn_del.setFixedHeight(32)
            btn_del.clicked.connect(lambda ch, cid=c["id"]: self.delete_cliente(cid))
